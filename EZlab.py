...
import devices.dev_Alicat as dev_Alicat
import devices.dev_K2000 as dev_K2000
import devices.dev_K2100 as dev_K2100
import devices.dev_K2182A as dev_K2182A
import devices.dev_Newport68945 as dev_Newport68945
import devices.dev_Newport69931 as dev_Newport69931
import devices.dev_RHUSB as dev_RHUSB
#import devices.dev_Chemyx as dev_Chemyx
#import devices.dev_PTC10 as dev_PTC10
#import devices.dev_ThorlabsSC10 as dev_ThorlabsSC10
#import devices.dev_K2400 as dev_K2400
#import devices.dev_LambdaSC as dev_LambdaSC


f_debug = True

available_driver = ['Alicat','K2100', 'K2000','Newport69931','Newport68945',
                    'RHUSB', 'K2182A']
    ...
    def check_config(self):
        ...
        for devidx, devkey in enumerate(list(self.config['Instruments'].keys())):
            ...
            if 'dev_driver' in self.config['Instruments'][devkey]:
                if self.config['Instruments'][devkey]['dev_driver'] not in available_driver:
        ...
        for devidx, devkey in enumerate(list(self.config['Instruments'].keys())):
            if self.config['Instruments'][devkey]['dev_enable']:
                ...
                if dev_driver == 'K2100':
                    ...
                    self.config['Instruments'][devkey]['GUI_thread'] = dev_K2100.driver_K2100(
                ...
                ###############################################################
                    ...
                    if f_debug:
                        print(' ... adding K2100 device ...')
                    # create the device thread
                    self.config['Instruments'][devkey]['GUI_thread'] = dev_K2000.driver_K2000(
                            self.config['Instruments'][devkey]
                            )
                    # start device thread
                    self.config['Instruments'][devkey]['GUI_thread'].start()
                ...
                ###############################################################
                    ...
                    # create the device thread
                    self.config['Instruments'][devkey]['GUI_thread'] = dev_K2182A.driver_K2182A(
                            self.config['Instruments'][devkey]
                            )
                    # start device thread
                    self.config['Instruments'][devkey]['GUI_thread'].start()
                ...
                #     # create the device thread
                #     self.config['Instruments'][devkey]['GUI_thread'] = dev_K2400.driver_K2400(
                #             self.config['Instruments'][devkey]
                #             )
                #     # start device thread
                #     self.config['Instruments'][devkey]['GUI_thread'].start()
                ...
                ###############################################################
                    ...
                    # create the device thread
                    self.config['Instruments'][devkey]['GUI_thread'] = dev_Newport69931.driver_Newport69931(
                            self.config['Instruments'][devkey]
                            )
                    # start device thread
                    self.config['Instruments'][devkey]['GUI_thread'].start()
                ...
                ###############################################################
                    ...
                    if f_debug:
                        print(' ... adding Newport69931 device ...')
                    # create the device thread
                    self.config['Instruments'][devkey]['GUI_thread'] = dev_Newport68945.driver_Newport68945(
                            self.config['Instruments'][devkey]
                            )
                    # start device thread
                    self.config['Instruments'][devkey]['GUI_thread'].start()
                ...
                ###############################################################
                    ...
                    # create the device thread
                    self.config['Instruments'][devkey]['GUI_thread'] = dev_RHUSB.driver_RHUSB(
                            self.config['Instruments'][devkey]
                            )
                    # start device thread
                    self.config['Instruments'][devkey]['GUI_thread'].start()
                ...
                ###############################################################
                    ...
                    for Alicatidx in range(len(self.config['Instruments'][devkey]['dev_id'])):
                        ...
                        if not 'GUI_thread' in self.config['Instruments'][devkey]:
                            # create the device thread
                            self.config['Instruments'][devkey]['GUI_thread'] = dev_Alicat.driver_Alicat(
                                self.config['Instruments'][devkey]
                                )
                            # start device thread
                            self.config['Instruments'][devkey]['GUI_thread'].start()
